...
console.log(`초기 농도 => ${concentration}`);
...
fetch(baseUrl)
  ...
  .then((data) => {
    ...
    console.log(`받아온 이후 농도 => ${concentration}`);
    App(concentration);
...
function App(concentration) {
  ...
  const conf = {
    ...
    size: 1.8,
    stiffness: 5,
    mouseRadius: `${concentration}`, // 휘날림 정도 조정은 여기서 하세요 베이비
    mouseStrength: 0.5 };
  ...
  function initCurtain() {
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vUv = uv;
          vColor = vec4(color, 0.1 + smoothstep(0.0, 0.5, uv.y) * 0.5); // 투명도는 여기서 바꾸세요 베이비
  ...
  function initListeners() {
    ...
    // document.body.addEventListener('click', updateColors, false); // 클릭시 색상 변경 listener
  }
